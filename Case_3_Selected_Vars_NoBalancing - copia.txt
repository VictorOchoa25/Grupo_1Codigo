# ==============================================================================
# SCRIPT 3: SELECTED VARIABLES / NO BALANCING
# ==============================================================================
...
# Output Folder
dir.create("Results_Case3_Selected_NoBal", showWarnings = FALSE)
...
# 5. MODELING LOOP
print(">> CASE 3 STARTING: SELECTED VARS / NO BALANCE")
...
for (method in model_list) {
  ...
  if (!is.null(fit)) {
    ...
    if(!is.null(roc_obj)) {
      ...
      sink(paste0("Results_Case3_Selected_NoBal/Res_", method, ".txt"))
      ...
      # Save model
      saveRDS(fit, file = paste0("Results_Case3_Selected_NoBal/model_", method, ".rds"))
    }
  }
}

# 6. SAVE AND DISPLAY RESULTS
print("=== FINAL RESULTS (SELECTED VARS, NO BALANCING) ===")
print(results_summary[order(-results_summary$AUC), ])

write.table(results_summary, 
            "Results_Case3_Selected_NoBal/Summary_Leaderboard.txt", 
            sep = "\t", 
            row.names = FALSE)

write.csv(results_summary, 
          "Results_Case3_Selected_NoBal/Summary_Leaderboard.csv", 
          row.names = FALSE)
...
if(nrow(results_summary) > 0) {
  ...
  p <- ggplot(results_summary, aes(x = reorder(Model, AUC), y = AUC, fill = AUC)) +
    ...
    labs(title = "Model Performance Comparison (Selected Variables, No Balancing)",
         subtitle = "Using only important features: failures, absences, higher, age, Medu, goout, guardian",
         x = "Model",
         y = "AUC") +
    theme_minimal() +
    ylim(0, 1)
  
  ggsave("Results_Case3_Selected_NoBal/Model_Comparison.png", 
         plot = p, width = 10, height = 6, dpi = 300)
  
  print("Model comparison plot saved.")
}

# 8. COMPARE WITH PREVIOUS CASES (if available)
compare_cases <- function() {
  comparisons <- list()
  
  if(file.exists("Results_Case1_All_NoBal/Summary_Leaderboard.csv")) {
    case1 <- read.csv("Results_Case1_All_NoBal/Summary_Leaderboard.csv")
    comparisons$Case1 <- case1
  }
  
  if(file.exists("Results_Case2_All_Balanced/Summary_Leaderboard.csv")) {
    case2 <- read.csv("Results_Case2_All_Balanced/Summary_Leaderboard.csv")
    comparisons$Case2 <- case2
  }
  
  if(length(comparisons) > 0) {
    print("=== COMPARISON ACROSS ALL CASES ===")
    
    # Combine all results
    all_results <- data.frame()
    
    if(!is.null(comparisons$Case1)) {
      case1_clean <- comparisons$Case1 %>%
        dplyr::select(Model, AUC, Sensitivity) %>%
        mutate(Case = "Case1: All Vars, No Balance")
      all_results <- rbind(all_results, case1_clean)
    }
    
    if(!is.null(comparisons$Case2)) {
      case2_clean <- comparisons$Case2 %>%
        dplyr::select(Model, AUC, Sensitivity) %>%
        mutate(Case = "Case2: All Vars, Balanced")
      all_results <- rbind(all_results, case2_clean)
    }
    
    case3_clean <- results_summary %>%
      dplyr::select(Model, AUC, Sensitivity) %>%
      mutate(Case = "Case3: Selected Vars, No Balance")
    all_results <- rbind(all_results, case3_clean)
    
    print(all_results)
    
    # Save comparison
    write.csv(all_results, 
              "Results_Case3_Selected_NoBal/Comparison_All_Cases.csv", 
              row.names = FALSE)
  }
}
